...
export async function validateAdminCredentials(login: string, password: string) {
  if (!config.adminLogin || !safeEqual(login, config.adminLogin)) return false;
...
export function verifyAdminToken(token?: string) {
  ...
  try {
    const data = JSON.parse(Buffer.from(payload, 'base64url').toString('utf8')) as { role?: string; login?: string; exp?: number };
    return data.role === 'admin' && data.login === config.adminLogin && Number(data.exp) > Date.now();
  } catch {
    return false;
  }
}
